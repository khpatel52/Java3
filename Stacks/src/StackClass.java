...
public class StackClass{
    ...
    //Part 4 Exercise #33a p.237 Inspector Method
    public DataElement inspector(int location){
        //check if stack is empty
        if(isEmptyStack())
            throw new StackException("Stack is Empty");
        boolean found = false;
        //x represents actual position in the current stack
        int x = this.stackTop-location;
        
        if(x < 0)
            throw new StackException("Location does not exist");
        
        if(list[x] == null) return null;
        else return list[x].getCopy();//location to be returned
